...
## Cómo funciona
1. Usuario descarga zip de plataforma (~5MB) + pone `packwiz-installer-bootstrap.jar` en `minecraft/`
2. `pre-launch.sh`/`.bat` → llama `install-java.sh`/`.bat` (descarga JRE si no existe) → `jre25/bin/java` → packwiz-installer → mods + Minecraft
3. Scripts switch: `switch-to-optifine`/`switch-to-celeritas` (wildcards version-agnostic)
...
- 8 scripts estáticos en `minecraft/` con wildcards
...
1. Copiar repo, limpiar .git/ scripts/ .github/
...
4. Zipear (~5MB cada uno, SIN JREs)
...
- `.gitignore` - exclude .github_token + minecraft/jre25/; trackea 8 scripts en minecraft/ + scripts/build-zips.sh
